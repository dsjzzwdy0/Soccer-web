...
MgrUser.openAddMgr = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Soccer.ctxPath + '/mgr/user_add'
    });
...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Soccer.ctxPath + '/mgr/user_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.roleAssign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Soccer.ctxPath + '/mgr/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        ...
        var operation = function(){
            ...
            var ajax = new $ax(Soccer.ctxPath + "/mgr/delete", function () {
                Soccer.success("删除成功!");
                MgrUser.table.refresh();
            }, function (data) {
                Soccer.error("删除失败!" + data.responseJSON.message + "!");
            });
            ajax.set("userId", userId);
            ajax.start();
        };

        Soccer.confirm("是否删除用户" + MgrUser.seItem.account + "?",operation);
    }
};
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(Soccer.ctxPath + "/mgr/freeze", function (data) {
            Soccer.success("冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            Soccer.error("冻结失败!" + data.responseJSON.message + "!");
        });
        ajax.set("userId", userId);
        ajax.start();
    }
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(Soccer.ctxPath + "/mgr/unfreeze", function (data) {
            Soccer.success("解除冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            Soccer.error("解除冻结失败!");
        });
...
MgrUser.resetPwd = function () {
    if (this.check()) {
        ...
        parent.layer.confirm('是否重置密码为111111？', {
            ...
            var ajax = new $ax(Soccer.ctxPath + "/mgr/reset", function (data) {
                Soccer.success("重置密码成功!");
            }, function (data) {
                Soccer.error("重置密码失败!");
            });
            ajax.set("userId", userId);
