window._config = {
	isDebug : location.hash.slice(1) === 'is-debug'
			|| [ 'localhost', 'dev.bootstrap-table.com' ]
					.indexOf(location.hostname) > -1,
	cdnUrl : 'https://unpkg.com/bootstrap-table@1.15.3/dist/',
...
function _getLink(file) {
	...
	if (!/^http/.test(file)) {
		...
		if (window._config.isDebug) {
			url = window._config.localUrl + file.replace(/\.min/, '') + '?t='
					+ (+new Date())
		}
	}
...
function _themeUpdate(data) {
	...
	if (/semantic.html$/.test(location.pathname)) {
		return data
				...
				.replace(/select class="form-control"/g,
						'select class="ui dropdown"')
				.replace(
						/'bootstrap-table.min.js'/,
						'\'bootstrap-table.min.js\',\n      \'themes/semantic/bootstrap-table-semantic.min.js\'')
				.replace(/'bootstrap-table.min.css'/,
						'\'themes/semantic/bootstrap-table-semantic.min.css\'')
	}
	if (/bulma.html$/.test(location.pathname)) {
		return data
				...
				.replace(/'bootstrap-table.min.css'/,
						'\'themes/bulma/bootstrap-table-bulma.min.css\'')
	}
	if (/materialize.html$/.test(location.pathname)) {
		return data
				...
				.replace('<i class="fa fa-heart"></i>',
						'<i class="material-icons">star</i>')
				.replace('<i class="fa fa-trash"></i>',
						'<i class="material-icons">clear</i>')
				.replace(
						/'bootstrap-table.min.js'/,
						'\'bootstrap-table.min.js\',\n      \'themes/materialize/bootstrap-table-materialize.min.js\'')
				.replace(/'bootstrap-table.min.css'/,
						'\'themes/materialize/bootstrap-table-materialize.min.css\'')
	}
	if (/foundation.html$/.test(location.pathname)) {
		return data
				...
				.replace(/'bootstrap-table.min.css'/,
						'\'themes/foundation/bootstrap-table-foundation.min.css\'')
	}
	return data
}
...
function _beautifySource(data) {
	...
	var obj = eval('('
			+ strings.join('').replace(/[^\u0000-\u007E]/g, '').replace(
					/^init\((.*)\)$/, '$1') + ')')
		...
		$.ajax({
			...
			success : function(_data) {
				...
				if (isSource) {
					...
					$('#example').html(
							data.replace(/data-toggle="table"/g,
