window._config = {
	isDebug : [ 'localhost', 'dev.bootstrap-table.com' ]
...
function initThemes() {
	...
	var $theme = $('[data-theme="' + window._config.theme + '"]').addClass(
			'active')
	...
	$('[data-show]').each(
			function() {
				$(this).toggle(
						$(this).data('show').split(',').indexOf(
								window._config.theme) > -1)
			})
}
...
function loadUrl(url_) {
	...
	if (window._config.isViewSource) {
		url = template + '.html?v=VERSION&view-source&url=' + url_
				+ '#view-source'
	}*/
	$('iframe').attr('src', url)
}
...
function doSearch() {
	var searchClient = window.algoliasearch('FXDJ517Z8G',
			'9b89c4a7048370f4809b0bc77b2564ac')
	...
	search.addWidget(window.instantsearch.widgets.searchBox({
		container : '#searchbox'
	}))

	search.addWidget(window.instantsearch.widgets.hits({
		container : '.hits-body',
		templates : {
			item : function(hit) {
				var search = ''
				...
				return [
						'<div class="link" data-href="' + hit.url + search
								+ hit.anchor + '">', '<div class="category">',
						hit.anchor.split('/')[0].slice(1), '</div>',
