...
<%@ tag import="com.loris.common.util.ToolUtil" %>
...
<div class="form-group">
    ...
    <div class="col-sm-9">
        <input class="form-control" id="${id}" name="${id}"
               <c:if test="${value != null && value != ''}">value="${ToolUtil.dateType(value)}"</c:if>
               <c:if test="${type != null && type != ''}">type="${type}"</c:if>
               <c:if test="${type == null || type == '' }">type="text"</c:if>
               <c:if test="${readonly != null && readonly != ''}">readonly="${readonly}"</c:if>
               <c:if test="${clickFun != null && clickFun != ''}">onclick="${clickFun}"</c:if>
               <c:if test="${style != null && style != ''}">style="${style}"</c:if>
               <c:if test="${disabled != null && disabled != ''}">disabled="${disabled}"</c:if>
        >
